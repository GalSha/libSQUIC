 ...
 */
__attribute__ ((visibility("default"))) void SQUIC_CPP_UTIL_memset_integer(integer *&buffer, integer length) {
 ...
 * @param[in]  length  The length of the buffer
 */
__attribute__ ((visibility("default"))) void SQUIC_CPP_UTIL_memset_double(double *&buffer, integer length) {
 ...
 * @param[in]  length  The length of the buffer
 */
__attribute__ ((visibility("default"))) void SQUIC_CPP_UTIL_memcopy_integer(integer *&buffer, integer *values, integer length) {
	buffer = new integer[length]();
 ...
 * @param[in]  length  The length of the buffer
 */
__attribute__ ((visibility("default"))) void SQUIC_CPP_UTIL_memcopy_double(double *&buffer, double *values, integer length) {
	buffer = new double[length]();
 ...
 * @param      buffer  The buffer
 */
__attribute__ ((visibility("default"))) void SQUIC_CPP_UTIL_memfree_integer(integer *&buffer) {
	delete [] buffer;
}
 ...
 * @param      buffer  The buffer
 */
__attribute__ ((visibility("default"))) void SQUIC_CPP_UTIL_memfree_double(double *&buffer) {
	delete [] buffer;
}
...
__attribute__ ((visibility("default"))) void SQUIC_CPP(
	...
	// kind of (incomplete) Cholesky decomposition: 0 (CHOLMOD), 1 (PARDISO), 2 (JANUS)
	int cholesky = 0;
	int coord_update;
	int inversion;
	...
	if (use_ZERO_M) {
		...
		sq->CSC_to_CustomeData(X_rinx, X_cptr, X_val, X_nnz, &X0);
		sq->CSC_to_CustomeData(W_rinx, W_cptr, W_val, W_nnz, &W0);
		...
		sq->CSC_to_CustomeData(X_rinx, X_cptr, X_val, X_nnz, &X0);
		sq->CSC_to_CustomeData(W_rinx, W_cptr, W_val, W_nnz, &W0);
		sq->CSC_to_CustomeData(M_rinx, M_cptr, M_val, M_nnz, &LambdaMatrix);
	...
	if (max_iter == 0) {
		...
		// The memory footprint is the same but the nnz values in the front of the array
		X_nnz = nnz_count_X;
		...
		// Passout out stats
		info_num_iter = sq->Stat.opt.size(); // Number of objective values evalutions is the number of iterations
		info_times[0] = sq->Stat.time_total;
		info_times[1] = sq->Stat.time_cov;

		info_times[2] = accumulate(sq->Stat.time_itr.begin(), sq->Stat.time_itr.end(), 0.0);
		info_times[3] = accumulate(sq->Stat.time_chol.begin(), sq->Stat.time_chol.end(), 0.0);
		info_times[4] = accumulate(sq->Stat.time_inv.begin(), sq->Stat.time_inv.end(), 0.0);
		info_times[5] = accumulate(sq->Stat.time_upd.begin(), sq->Stat.time_upd.end(), 0.0);
