...
integer SQUIC::logDet(double *D, double *ld, double drop_tol)
{
	...
	err = Factorize_LDL(&X, &L, &P, D, drop_tol);
...
// generic entry point for LDL factorization if the matrix is given in CSC format
// if successful, return the Cholesky factor along with the reordering in L_factor, D_LDL and L_perm,
// where L_factor is unit lower triangular, D_LDL is the diagonal part s.t. A(perm,perm)=LDL'
integer SQUIC::Factorize_LDL(integer *A_rowind, integer *A_p, double *A_val,
							 ...
							 double *D_LDL, double drop_tol)
{
	...
	for (j = 0; j < nc; j++)
	{
	  ...
	  // (CHOL_SUITESPARSE==RunTimeConfig.factorization)
		...
		// Factorize_CHOLMOD(ja , ia, a, W.nc, W.nr, W.nnz , D_LDL);
		// generic CHOLMOD driver
		return Factorize_CHOLMOD(A_rowind, A_p, A_val, nc, nr, nnz, L_factor, L_perm, D_LDL);
}; // end Factorize_LDL

// generic entry point for LDL factorization if the matrix is given in SparseMatrix format
// if successful, return the Cholesky factor along with the reordering in L_factor, D_LDL and L_perm,
// where L_factor is unit lower triangular, D_LDL is the diagonal part s.t. A(perm,perm)=LDL'
integer SQUIC::Factorize_LDL(SparseMatrix *A, SparseMatrix *L_factor, SparseMatrix *L_perm, double *D_LDL, double drop_tol)
{
	...
	integer rtrn;
	// (CHOL_SUITESPARSE==RunTimeConfig.factorization)
	rtrn = Factorize_CHOLMOD(A_rowind, A_p, A_val, A->nc, A->nr, A->nnz, L_factor, L_perm, D_LDL);
...
// export LDL factorization generated from a Cholesky factorization along with the reordering
// where L_factor is unit lower triangular, D_LDL is the diagonal part s.t. A(perm,perm)=LDL'
void SQUIC::Export_LDL(double *D_LDL)
{
  // (CHOL_SUITESPARSE==RunTimeConfig.factorization)
		Export_CHOLMOD(D_LDL);
}; // end Export_LDL

// export block LDL factorization generated from a Cholesky factorization along with the reordering
// where L_factor is unit lower triangular, D_LDL is the diagonal part s.t. A(perm,perm)=LDL'
void SQUIC::BlockExport_LDL()
{
...
// if successful, return the Cholesky factor along with the reordering in L_factor, D_LDL and L_perm,
// where L_factor is unit lower triangular, D_LDL is the diagonal part s.t. A(perm,perm)=LDL'
integer SQUIC::Factorize_CHOLMOD(integer *A_rowind, integer *A_p, double *A_val, integer nc, integer nr, integer nnz, SparseMatrix *L_factor, SparseMatrix *L_perm, double *D_LDL)
{
...
#pragma omp critical
	{
		...
		// printf ("norm (A,1)   = %g\n", CHOLMOD_NORM_SPARSE(Cholmod.pA, 1, &Cholmod.common_parameters)) ;
		...
		if (analyze_done && L_perm != NULL)
		{
			...
			Cholmod.pL = CHOLMOD_ANALYZE_P(Cholmod.pA, perm, f, Cholmod.pA->ncol, &Cholmod.common_parameters);
			// CHOLMOD_CHANGE_FACTOR(CHOLMOD_REAL, 1, 0, 0, 1, Cholmod.pL, &Cholmod.common_parameters);

			double beta[2];
			beta[0] = 0;
			beta[1] = 0;

			CHOLMOD_FACTORIZE_P(Cholmod.pA, beta, f, Cholmod.pA->ncol, Cholmod.pL, &Cholmod.common_parameters);

			delete[] perm;
			delete[] f;
		}
		else
		{ // symbolic analysis is not yet available
			...
			// CHOLMOD_PRINT_FACTOR(Cholmod.pL,"Cholmod.pL pre",&Cholmod.common_parameters);
			CHOLMOD_FACTORIZE(Cholmod.pA, Cholmod.pL, &Cholmod.common_parameters);
			// CHOLMOD_PRINT_FACTOR(Cholmod.pL,"Cholmod.pL post",&Cholmod.common_parameters);
		}
		if (RunTimeConfig.verbose > 2)
		{
			//CHOLMOD_PRINT_COMMON("Cholmod.common_parameters", &Cholmod.common_parameters);
...
// if successful, return the Cholesky factor along with the reordering in L_factor, D_LDL and L_perm,
// where L_factor is unit lower triangular, D_LDL is the diagonal part s.t. A(perm,perm)=LDL'
void SQUIC::Export_CHOLMOD(double *D_LDL)
{
	...
	if (!Cholmod.pL->is_super)
	{
		// (partially) export Cholesky factor in SparseMatrix format as well as the diagonal part of LDL
		integer index = 0;
		for (integer j = 0; j < Cholmod.pL->n; j++)
		{
			...
			L.ncol[j] = nz;
			L.rowind[j] = (integer *)realloc(L.rowind[j], (size_t)nz * sizeof(integer));
	...
	{ // is_super
		...
		for (integer s = 0; s < nblocks; s++)
		{
			...
			for (integer i = 0; i < nscol; i++, j += nsrow + 1)
			{
				...
				L.rowind[cnt + i] = (integer *)realloc(L.rowind[cnt + i], (size_t)(nsrow - i) * sizeof(integer));
				L.val[cnt + i] = (double *)realloc(L.val[cnt + i], (size_t)(nsrow - i) * sizeof(double));
...
// CHOLMOD entry point
// if successful, return the Cholesky factor along with the reordering in L_factor, D_LDL and L_perm,
// where L_factor is BLOCK unit lower triangular, BiD is inverse BLOCK diagonal part s.t. A(perm,perm)=LDL'
void SQUIC::BlockExport_CHOLMOD()
{
	...
	if (!Cholmod.pL->is_super)
	{
		...
		for (integer s = 0; s < nblocks; s++)
		{
			...
			if (BiD.colind[s] != NULL)
				BiD.colind[s] = (integer *)realloc(BiD.colind[s], (size_t)1 * sizeof(integer));
			else
				BiD.colind[s] = (integer *)malloc((size_t)1 * sizeof(integer));

			// (re-)allocate memory for numerical values in the diagonal block
			if (BiD.valD[s] != NULL)
				BiD.valD[s] = (double *)realloc(BiD.valD[s], (size_t)1 * 1 * sizeof(double));
			else
			...
			if (Cholmod.pL->is_monotonic)
			{
				...
				// start position numerical values of the sub-block-diagonal part in block s
			...
			{ // indices are not sorted in increasing order
	...
	else
	{
		...
		// cout << "is_monotonic: " << Cholmod.pL->is_monotonic << endl; fflush(stdout);
		...
		for (integer s = 0; s < nblocks; s++)
		{
			...
			// starting point of the strict lower triangular part in the supernode panel
			// relative to L->s, L->x
			...
			// (re-)allocate memory for column indices
			if (BiD.colind[s] != NULL)
				BiD.colind[s] = (integer *)realloc(BiD.colind[s], (size_t)nscol * sizeof(integer));
			else
				BiD.colind[s] = (integer *)malloc((size_t)nscol * sizeof(integer));

			// (re-)allocate memory for numerical values in the diagonal block
			if (BiD.valD[s] != NULL)
				BiD.valD[s] = (double *)realloc(BiD.valD[s], (size_t)nscol * nscol * sizeof(double));
			else
			...
			if (info)
			{
				if (info < 0)
					...
					printf("LAPACK's dpotri_ routine encountered zero column in step %ld\n", info);
			}
			...
			// (re-)allocate memory for column indices
			if (BL.colind[s] != NULL)
				BL.colind[s] = (integer *)realloc(BL.colind[s], (size_t)nscol * sizeof(integer));
			else
				BL.colind[s] = (integer *)malloc((size_t)nscol * sizeof(integer));

			// (re-)allocate memory for sub-diagonal row indices
			if (BL.rowind[s] != NULL)
				BL.rowind[s] = (integer *)realloc(BL.rowind[s], (size_t)nsrow2 * sizeof(integer));
			else
				BL.rowind[s] = (integer *)malloc((size_t)nsrow2 * sizeof(integer));

			// (re-)allocate memory for numerical values in the sub-diagonal block
			if (BL.valE[s] != NULL)
				BL.valE[s] = (double *)realloc(BL.valE[s], (size_t)nsrow2 * nscol * sizeof(double));
			else
			...
			// start position row indices in block s
			pi = BL.rowind[s];
			// copy entries initially to a buffer in order to efficiently use GEMM later
			pval = BL.valE[s];
			...
			if (nsrow2 > 0)
			{
				...
				dtrsm_("r", "l", "n", "n", &mynsrow2, &mynscol, &alpha, Lx + psx, &mynsrow, BL.valE[s], &mynsrow2, 1, 1, 1, 1);
#else
				dtrsm_("r", "l", "n", "n", &nsrow2, &nscol, &alpha, Lx + psx, &nsrow, BL.valE[s], &nsrow2, 1, 1, 1, 1);
#endif
			} // end if
			nnzBL += nsrow2 * nscol;
