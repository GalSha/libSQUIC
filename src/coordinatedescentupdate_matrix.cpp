...
// We assume that all \Delta_ij, \Delta_ji have existing (possibly 0) entries to
// cover the changes slightly improved version using partiall OpenMP
void SQUIC::BlockCoordinateDescentUpdate(
    const double Lambda, SparseMatrix *LambdaMatrix, SparseMatrix *D,
    integer *idx, integer *idxpos, double *val, index_pair *activeSet,
    integer first, integer last, double &l1norm_D, double &diffD) {
  /*
...
val     /  product \Delta W_{:,j}; initially we assume that idxpos and val are
zero activeSet  active set of positions (i,j) for descent update first,last
l1norm_D       1/2 |\Delta|_1
...
*/
  ...
  // we scan the sparse matrices for certain values at position (i,j), (i,i),
  // (j,j) if these are not present, then their values must be zero
  ...
  // i<=j, the matrices A,B,C will be upper triangular, whereas M will be
  // strictly lower triangular, i.e., the diagonal part in the transposed
  // pattern is excluded
  SparseMatrix A, B, C, M;
  integer Ancol, *pArowind, Bncol, *pBrowind, Cncol, *pCrowind, Mncol,
      *pMrowind, Dncol, *pDrowind, Sncol, *pSrowind, Wncol, *pWrowind, Xncol,
      *pXrowind;
  double *pAval, *pBval, *pCval, *pMval, *pDval, *pSval, *pWval, *pXval;
...
#pragma omp parallel for shared(p, B, S, diagW, W, A) private(                \
    m, k, i, Bncol, pBrowind, pBval, Sncol, pSrowind, pSval, Wncol, pWrowind, \
    pWval, pAval)
  for (j = 0; j < p; j++) {
    // B <- S - W + W' * \Delta * W
...
#ifdef PRINT_INFO_BCDU
  printf(
      "B, 1. step completed, W, diagonal part extracted, A,1 . step, B, 2. "
      "step completed, %8.1le [sec]\n",
      omp_get_wtime() - timeBegin);
  fflush(stdout);
  timeBegin = omp_get_wtime();
#endif

#pragma omp parallel for shared(p, A, diagW, X, D, C) private(            \
    m, i, k, l, Xij, Dij, Ancol, pArowind, pAval, Cncol, pCrowind, pCval, \
...
#define RANDOMIZE
  ...
  // --------------------- MAIN LOOP -----------------------
  // B <- S - W + W' * \Delta * W
  // 3. step: finally we update B by B <- B + W' * \Delta * W on the pattern of
  // B interlace the computation with updating Delta using soft-thresholding,
  // column-by-column
  ...
  for (jj = 0; jj < p; jj++) {
    ...
    if (Bncol > 0) {
      ...
      // to do so we build sparse linear combinations U <- \sum_l \Delta_{:,l}
      // W_lj of the columns of \Delta
      // !!! also include M, since M covers the updates of \Delta for its strict
      // !!! lower triangular part that has not yet been updated
      // !!! thus also build sparse linear combinations U <- U+\sum_l M_{:,l}
      // W_lj
      Wncol = W.ncol[j];
      pWrowind = W.rowind[j];
      pWval = W.val[j];
...
#ifdef PRINT_INFO1
      for (k = 0; k < cnt; k++) printf("%2d: %8.1le\n", idx[k], val[k]);
#endif
      ...
      // now sort idx and val simultaneously such that idx is sorted in
      // increasing order use idxpos as stack
      qsort1_(val, idx, idxpos, &cnt);
      // clean up stack
      for (k = 0; k < cnt; k++) idxpos[k] = 0;
#ifdef PRINT_INFO1
      for (k = 0; k < cnt; k++) printf("%2d: %8.1le\n", idx[k], val[k]);
#endif
#ifdef PRINT_INFO_BCDU
#pragma omp single
      ...
      for (kk = 0; kk < Bncol; kk++) {
        ...
        pWrowind = W.rowind[i];
        pWval = W.val[i];
        // now compute sparse-sparse scalar product W_{:,i}^T*val and update
        // B_ij recall that the sparse vectors have ascending indices
        k = l = 0;
        while (k < cnt && l < Wncol) {
          ...
          if (ld < lw)
            ...
            l++;
          else {  // indices match, update b by scalar product contribution
            d += pWval[l] * val[k];
            k++;
            l++;
          }  // end if-elseif-else
        ...
        pBval[kk] = d;
        // now we have finally achieved B_ij = S_ij - W_ij + W_{:,i}^T \Delta
        // W_{:,j}
#ifdef PRINT_INFO1
...
#pragma omp single
      ...
      for (k = 0; k < cnt; k++) val[k] = 0.0;

      // now that we have commonly computed A, B, C we start updating Delta
      // using soft-thresholding note that at this stage, only column j of B is
      // correctly computed, i.e., the second order term W_{:,i}'\Delta W_{:,j}
      // has only been added to columns j in {colind[0],...,colind[jj]} of B.
      // Changes in \Delta_ij by some mu require to update C_ij <- C_ij+mu as
      // well as B_{rj} <- B_{rj} + mu (W_{ir}W_{jj}+W_{rj}W_{ij}) for all r>i
      // such that B_{rj} is available and such that W_{ir} or (W_{jr} and
      // W_{ij}) are available. This will make the information in B_{:,j} become
      // consistent again B_{:,colind[s]} such that s<jj are not considered
      // anymore, similarly B_{rj} s.t. r<=i. B_{:,colind[s]} s.t. s>jj have not
      // been computed anyway, thus these columns need not be updated either
      m = k = 0;
...
#ifdef PRINT_INFO1
            printf("diffD=%8.1le, l1norm_D =%8.1le\n", diffD, l1norm_D);
#endif
            // theoretically we need to update C
            // updating C <- C + mu (e_ie_j^T + e_je_i)^T
            // this only affects the upper triangular part, since the lower
            // triangular part is not stored anyway, but the upper triangular
            // part is not referenced anymore in future computations, therefore
            // we can leave it as it is C.val[j][m]+=mu;

            // now also update B
            // updating B <- B + mu (W_{i,:}^T (e_ie_j^T+e_je_i^T) W_{j,:})
            //             = B + mu (W_{i,:}^TW_{j,:}+W_{j,:}^TW_{i,:}) in
            //             column j, i.e.,
...
#ifdef PRINT_INFO_BCDU
  printf(
      "Delta, upper triangular part updated, %8.1le [sec], loop1: %8.1le "
      "[sec], loop2: %8.1le [sec]\n",
...
#endif
  // for symmetry reasons also update \Delta_{ji} using the transposed values
  // stored in M but skip diagonal part in order to avoid duplicate updates
#pragma omp parallel for shared(p, M, D) private( \
    m, k, i, mu, Mncol, pMrowind, pMval, Dncol, pDrowind, pDval)
  for (j = 0; j < p; j++) {
    m = k = 0;
    Mncol = M.ncol[j];
    pMrowind = M.rowind[j];
    ...
    while (m < Mncol && k < Dncol) {
      i = pMrowind[m];
      if (i != j) {  // should be satisfied by contruction
        mu = pMval[m];
        // find out whether \Delta_ij exists
        while (k < Dncol) {
          if (pDrowind[k] >= i) {
