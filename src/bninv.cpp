...
#ifndef _BLOCK_NEUMANN_INVERSION_
#define _BLOCK_NEUMANN_INVERSION_
...
/* ====================================================================
   ...
   ====================================================================
    ...
    $Id: bfspaisym.c 4061 2018-03-04 21:38:56Z bolle $
...
void SQUIC::BlockNeumannInv(double droptol, double *SL)

{
  integer i, j, k, l, m, n, r, s, t, nnz, flag, *ia, *ja, *ja2, cnt, cnt2, cntb,
      ss, tt, ttt, i2, i3, jj, ii, kk, r2, nblocks, *block, ncols, mrows, *idxl,
      *idxl2, *idxposl, **idxl_src, **idxl2_src, **idxposl_src, *idxb, *idxbpos,
      **idxb_src, **idxbpos_src, *iLTbegin, **iLTbegin_src, mythreadnum,
      iL_inplace, iL_new_inplace, BiD_inplace;
  char *transa, *transb;
  size_t *iL_size_rowind, *iL_size_valE, *size_vall_src, *size_vall2_src,
      *iL_new_size_rowind, *iL_new_size_valE;
  double nrm, *nrml, mytol, buff, *valr;
  double *a, alpha, beta, *pr, *Ainv_valuesR, *vall, **vall_src, *vall2,
      **vall2_src;
#ifdef _PROFILING_
  double timeBegin, timeBeginLocal, time_bfspai,
      time_neumann = 0.0, time_matrix_matrix = 0.0, time_matrix_matrix1 = 0.0,
      time_matrix_matrix2 = 0.0, time_iLT;
  ...
  if (RunTimeConfig.verbose > 2) {
    printf("- use tolerance %8.1le for approximate inversion\n", mytol);
    fflush(stdout);
  }  // end if
  ...
  if (SL != NULL) {
    printf("SL:\n");
    fflush(stdout);
    for (i = 0; i < n; i++) printf("%12.4le", SL[i]);
    printf("\n");
    fflush(stdout);
  }
  printf("p:\n");
  ...
  iL.valD = NULL;
  iL.valE = (double **)malloc((size_t)nblocks * sizeof(double *));
  ...
  iL_new.valD = NULL;
  iL_new.valE = (double **)malloc((size_t)nblocks * sizeof(double *));
...
#pragma omp parallel for shared(nblocks, BL, iL, block, iL_size_rowind,   \
                                iL_size_valE, iL_new, iL_new_size_rowind, \
                                iL_new_size_valE,                         \
                                n) private(ja, ncols, k, i, mrows, a)
  for (j = 0; j < nblocks; j++) {
    ...
    for (k = 0; k < ncols; k++) {
      ...
      if (i < 0 || i >= n)
        printf(
            " 1. scalar index out of range when referring to block[%ld]=%ld\n",
            i, j);
      fflush(stdout);
      if (j < 0 || j >= nblocks)
        printf(
            " 1. block index out of range when referring to block[%ld]=%ld\n",
            i, j);
      fflush(stdout);
#endif
    }  // end for k
    ...
    iL.valE[j] = (double *)malloc(iL_size_valE[j] * sizeof(double));
    memcpy(iL.valE[j], BL.valE[j], mrows * ncols * sizeof(double));
    // switch sign
    k = mrows * ncols;
    a = iL.valE[j];
    for (i = 0; i < k; i++, a++) *a = -*a;
...
#ifdef PRINT_INFO
  printf("BNINV: iL=I-tril(L,-1) done\n");
  fflush(stdout);
  // PrintBlockSparse(&iL);fflush(stdout);
#endif

  // arrays idxl, idxposl and vall, vall2 are used to uncompress sparse arrays
  // and in particular idxposl is used to avoid duplicate entries when more than
  ...
  vall_src = (double **)malloc((size_t)k * sizeof(double *));
  vall2_src = (double **)malloc((size_t)k * sizeof(double *));
  nrml = (double *)malloc((size_t)k * sizeof(double));
  ...
  for (i = 0; i < k; i++) {
    ...
    vall_src[i] = (double *)malloc(size_vall_src[i] * sizeof(double));
    size_vall2_src[i] = n * ELBOW;
    vall2_src[i] = (double *)malloc(size_vall2_src[i] * sizeof(double));
  }  // end for i
  ...
  while (flag) {
    ...
    k = omp_get_max_threads();
    for (i = 0; i < k; i++) nrml[i] = 0.0;

      // Horner scheme iL_new <- iL*(I-PREC.BL)+I
#pragma omp parallel for shared(                                             \
    nblocks, n, BL, idxl_src, idxl2_src, idxposl_src, vall_src, vall2_src,   \
    block, iL_new, iL_new_size_rowind, iL_new_size_valE, size_vall_src, iL,  \
    size_vall2_src, nrml) private(mythreadnum, idxl, idxl2, idxposl, vall,   \
                                  vall2, r, ncols, mrows, ia, cnt, l, s, r2, \
                                  cnt2, ja, jj, t, k, ii, i2, tt, a, transa, \
                                  transb, alpha, beta, buff, pr, iL_inplace, \
                                  iL_new_inplace)
    for (j = 0; j < nblocks; j++) {
      // who am I
...
#ifdef PRINT_CHECK
      for (r = 0; r < n; r++) {
        if (idxposl[r]) printf("idxposl[%ld] still dirty\n", r + 1);
        idxposl[r] = 0;
      }  // end for r
#endif
      ...
      while (r < mrows) {
        ...
        if (l < 0 || l >= n)
          printf(
              " 2. scalar index out of range when referring to "
              "%ld=block[%ld]\n",
              s, l);
        fflush(stdout);
        if (s < 0 || s >= nblocks)
          printf(
              " 2. block index out of range when referring to %ld=block[%ld]\n",
              s, l);
        fflush(stdout);
#endif
        ...
        while (r2 < mrows) {
          ...
          if (l < 0 || l >= n)
            printf(
                " 3. scalar index out of range when referring to "
                "%ld!=block[%ld]\n",
                s, l);
          fflush(stdout);
          if (s < 0 || s >= nblocks)
            printf(
                " 3. block index out of range when referring to "
                "%ld!=block[%ld]\n",
                s, l);
          fflush(stdout);
#endif
      ...
      if (iL_new.rowind[j] == NULL) {
        ...
        iL_new.rowind[j] =
            (integer *)malloc(iL_new_size_rowind[j] * sizeof(integer));
      } else {
        iL_new_size_rowind[j] = MAX(cnt, iL_new_size_rowind[j]);
        iL_new.rowind[j] = (integer *)realloc(
            iL_new.rowind[j], iL_new_size_rowind[j] * sizeof(integer));
      }
      if (iL_new.valE[j] == NULL) {
        iL_new_size_valE[j] = (2 * cnt + 1 + ELBOW) * ncols;
        iL_new.valE[j] = (double *)malloc(iL_new_size_valE[j] * sizeof(double));
      } else {
        iL_new_size_valE[j] = MAX(cnt * ncols, iL_new_size_valE[j]);
        iL_new.valE[j] = (double *)realloc(
            iL_new.valE[j], iL_new_size_valE[j] * sizeof(double));
      }

      // init numerical values
      a = iL_new.valE[j];
      k = cnt * ncols;
      for (r = 0; r < k; r++) a[r] = 0.0;
      ...
      while (r < mrows) {
        ...
        if (l < 0 || l >= n)
          printf(
              " 4. scalar index out of range when referring to "
              "%ld=block[%ld]\n",
              s, l);
        fflush(stdout);
        if (s < 0 || s >= nblocks)
          printf(
              " 4. block index out of range when referring to %ld=block[%ld]\n",
              s, l);
        fflush(stdout);
#endif
        // make sure that this block column has not been considered yet
        ...
        while (r2 < mrows) {
          ...
          if (l < 0 || l >= n)
            printf(
                " 5. scalar index out of range when referring to "
                "%ld!=block[%ld]\n",
                s, l);
          fflush(stdout);
          if (s < 0 || s >= nblocks)
            printf(
                " 5. block index out of range when referring to "
                "%ld!=block[%ld]\n",
                s, l);
          fflush(stdout);
#endif
...
#ifdef PRINT_INFO2
          printf(
              "iL{%ld} need not be copied, since %ld...%ld are %ld contiguous "
              "indices\n",
              s + 1, ia[r] + 1, ia[r2 - 1] + 1, r2 - r);
          fflush(stdout);
#endif
        } else
          iL_inplace = 0;
        ...
        if (jj > 0) {
          ...
          if (iL_inplace) {
            ...
            size_vall_src[mythreadnum] =
                MAX(size_vall_src[mythreadnum], jj * (r2 - r));
            vall = vall_src[mythreadnum];
            vall_src[mythreadnum] = (double *)realloc(
                vall, size_vall_src[mythreadnum] * sizeof(double));
            ...
            for (l = r; l < r2; l++, vall += jj) {
              ...
              memcpy(vall, a, jj * sizeof(double));
            }  // end for l
            vall = vall_src[mythreadnum];
...
#ifdef PRINT_INFO2
            printf(
                "block %ld of iL_new{%ld} need not be copied, since %ld...%ld "
                "are %ld contiguous indices\n",
                s + 1, j + 1, ja[0] + 1, ja[jj - 1] + 1, jj);
            fflush(stdout);
            /*
...
*/
          ...
          if (ncols == 1 && r2 - r == 1) {
            ...
            if (iL_new_inplace) {
              ...
              int myjj = jj, myl = l;
              daxpy_(&myjj, &alpha, vall, &myl, a, &myl);
#else
              daxpy_(&jj, &alpha, vall, &l, a, &l);
#endif
            }       // end if iL_new_inplace
            else {  // we need to perform a sparse AXPYI
#ifdef _USE_MKL_
                    // use sparse AXPYI iL_new{j}[idxl2]= -pr[r] * vall +
                    // iL_new{j}[idxl2]
              for (l = 0; l < jj; l++) {
...
#ifdef _BLAS_LAPACK_32_
              int myjj = jj, mykk,
                  *myidxl2 = (int *)malloc((size_t)jj * sizeof(int));
              for (mykk = 0; mykk < myjj; mykk++) myidxl2[mykk] = idxl2[mykk];
              cblas_daxpyi(myjj, alpha, vall, myidxl2, a);
              free(myidxl2);
#else
              cblas_daxpyi(jj, alpha, vall, idxl2, a);
#endif
	  ...
	  // update iL_new{j}[idxposl[ja]] += -pr[r] * vall
          ...
          // level-3 BLAS GEMM case
            ...
            if (iL_new_inplace) {
              ...
              fflush(stdout);
#endif
            } else {  // we need to copy the rows of iL_new{j} associated with
                      // block s
#ifdef PRINT_INFO2
              printf("perform gather\n");
              fflush(stdout);
#endif
              size_vall2_src[mythreadnum] =
                  MAX(size_vall2_src[mythreadnum], jj * ncols);
              vall2 = vall2_src[mythreadnum];
              vall2_src[mythreadnum] = (double *)realloc(
                  vall2, size_vall2_src[mythreadnum] * sizeof(double));
              vall2 = vall2_src[mythreadnum];
              for (l = 0; l < jj; l++) {
                // required row from block s of iL
                i2 = ja[l];
              ...
              for (k = 0; k < ncols; k++, vall2 += jj, a += cnt) {
                ...
                int myjj = jj, mykk,
                    *myidxl2 = (int *)malloc((size_t)jj * sizeof(int));
                for (mykk = 0; mykk < myjj; mykk++) myidxl2[mykk] = idxl2[mykk];
                cblas_dgthr(myjj, a, vall2, myidxl2);
                free(myidxl2);
#else
                cblas_dgthr(jj, a, vall2, idxl2);
#endif
#else
                for (l = 0; l < jj; l++) {
                  // required row from block s of iL
            ...
            if (iL_new_inplace) {
              ...
              fflush(stdout);
              for (i2 = 0; i2 < jj; i2++) {
                for (k = 0; k < ncols; k++) printf("%12.4le", a[i2 + k * cnt]);
                printf("\n");
                fflush(stdout);
              }
#endif
#ifdef _BLAS_LAPACK_32_
              int myjj = jj, myncols = ncols, myl = l, mymrows = mrows,
                  mycnt = cnt;
              dgemm_(transa, transb, &myjj, &myncols, &myl, &alpha, vall, &myjj,
                     pr + r, &mymrows, &beta, a, &mycnt, 1, 1);
#else
              dgemm_(transa, transb, &jj, &ncols, &l, &alpha, vall, &jj, pr + r,
                     &mrows, &beta, a, &cnt, 1, 1);
#endif
            } else {
#ifdef _BLAS_LAPACK_32_
              int myjj = jj, myncols = ncols, myl = l, mymrows = mrows;
              dgemm_(transa, transb, &myjj, &myncols, &myl, &alpha, vall, &myjj,
                     pr + r, &mymrows, &beta, vall2, &myjj, 1, 1);
#else
              dgemm_(transa, transb, &jj, &ncols, &l, &alpha, vall, &jj, pr + r,
                     &mrows, &beta, vall2, &jj, 1, 1);
#endif
            }  // end else
            ...
            if (!iL_new_inplace) {
              ...
              for (k = 0; k < ncols; k++, vall2 += jj, a += cnt) {
                ...
                int myjj = jj, mykk,
                    *myidxl2 = (int *)malloc((size_t)jj * sizeof(int));
                for (mykk = 0; mykk < myjj; mykk++) myidxl2[mykk] = idxl2[mykk];
                cblas_dsctr(myjj, vall2, myidxl2, a);
                free(myidxl2);
#else
                cblas_dsctr(jj, vall2, idxl2, a);
#endif
#else
                for (l = 0; l < jj; l++) {
                  // required row from block s of iL
      ...
      for (r = 0; r < jj; r++) {
        ...
        if (t) {  // yes
          ...
          idxposl[l] = -1;
        } else  // whoops! a drop out case, this should rarely happen, maybe
                // never
      ...
      if (ncols == 1) {
        ...
        for (r = 0; r < cnt; r++) {
          ...
          if (idxposl[l] > 0) nrml[mythreadnum] = MAX(nrml[mythreadnum], buff);
          // reset buffer
          idxposl[l] = 0;
        }     // end for r
        ...
        for (r = 0; r < cnt; r++) {
          // new row index l
          l = idxl[r];
          // maximum entry in row r of iL_new{j} physically starting at position
          // r
#ifdef _BLAS_LAPACK_32_
          int myncols = ncols, mycnt = cnt;
          ii = idamax_(&myncols, vall + r, &mycnt) - 1;
#else
          ii = idamax_(&ncols, vall + r, &cnt) - 1;
#endif
          // flag index if the value is sufficiently small
          buff = FABS(vall[r + ii * cnt]);
          if (buff <= 0.1 * mytol) idxl[r] = -1;
...
#pragma omp parallel for shared(nblocks, iL, iL_new, iL_size_rowind, \
                                iL_new_size_rowind, iL_size_valE,    \
                                iL_new_size_valE) private(i, a, ja, kk)
    for (j = 0; j < nblocks; j++) {
      i = iL.nblockrow[j];
    ...
    k = omp_get_max_threads();
    nrm = 0.0;
    for (i = 0; i < k; i++) nrm = MAX(nrml[i], nrm);

    // increase number of power in the Neumann series
    m++;
#ifdef PRINT_INFO
    printf("BNINV: power m=%2ld, nrm=%8.1le\n", m, nrm);
    fflush(stdout);
#endif
    if (RunTimeConfig.verbose > 2) {
      integer myiTemp = 0;
      for (integer j = 0; j < nblocks; ++j) {
        myiTemp += (iL_new.nblockrow[j] + .5 * iL_new.nblockcol[j] + .5) *
                   iL_new.nblockcol[j];
      }  // end for j

      printf(
          "- BNINV: power m=%2ld, nrm=%8.1le, nnz(iL)/p=%8.1le, "
          "nnz(L)/p=%8.1le\n",
          m, nrm, myiTemp / (double)p,
          (BL.nnz + 0.5 * BiD.nnz + 0.5 * p) / (double)p);
      fflush(stdout);
    }  // end if
...
#pragma omp parallel for shared(nblocks, iL, mytol) private( \
    vall, ja, mrows, ncols, r, l, ii, buff, pr, k, cnt, alpha)
  for (j = 0; j < nblocks; j++) {
#ifdef PRINT_INFO2
    ...
    if (ncols == 1) {
...
#ifdef _BLAS_LAPACK_32_
        int myncols = ncols, mymrows = mrows;
        ii = idamax_(&myncols, vall + r, &mymrows) - 1;
#else
        ii = idamax_(&ncols, vall + r, &mrows) - 1;
#endif
        // flag index if the value is sufficiently small
        buff = FABS(vall[r + ii * mrows]);
#ifdef PRINT_INFO2
        printf("||L(%ld,:)||=|L(%ld,%ld)|=%12.4le\n", l + 1, l + 1,
               iL.colind[j][ii] + 1, buff);
        fflush(stdout);
#endif
        if (buff <= mytol) {
#ifdef PRINT_INFO2
      ...
      fflush(stdout);
      for (r = 0; r < cnt; r++) printf("%12ld", ja[r] + 1);
      printf("\n");
      fflush(stdout);
#endif
#ifdef PRINT_INFO2
      printf("3. pass completed, thread=%ld, j=%ld, mrows=%ld\n",
             omp_get_thread_num(), j, cnt);
      fflush(stdout);
#endif
    }  // end if-else ncols=1
#ifdef PRINT_INFO
...
#endif
    iL.rowind[j] =
        (integer *)realloc(iL.rowind[j], (size_t)cnt * sizeof(integer));
    iL.valE[j] =
        (double *)realloc(iL.valE[j], (size_t)cnt * ncols * sizeof(double));
  }  // end for j
     // end OMP parallel for
  ...
  printf("BNINV: approximate inverse sparsified, nnz=%ld\n", jj);
  fflush(stdout);
  // PrintBlockSparse(&iL);fflush(stdout);
#endif
  ...
  // compute W=S*P*iL^T*BiD*iL*P^T*S as follows
  ...
  // 3. rescale iL_new and sparsify it when transferring it to W
  // 4. complete W with its transpose for the MATLAB interface data
  SparseMatrix iLT;
  ...
  for (j = 0; j < nblocks; j++) {
    ...
    while (r < mrows) {
      ...
      if (l < 0 || l >= n)
        printf(
            " 6. scalar index out of range when referring to %ld=block[%ld]\n",
            s, l);
      fflush(stdout);
      if (s < 0 || s >= nblocks)
        printf(
            " 6. block index out of range when referring to %ld=block[%ld]\n",
            s, l);
      fflush(stdout);
#endif
      ...
      while (r2 < mrows) {
        ...
        if (l < 0 || l >= n)
          printf(
              " 7. scalar index out of range when referring to "
              "%ld!=block[%ld]\n",
              s, l);
        fflush(stdout);
        if (s < 0 || s >= nblocks)
          printf(
              " 7. block index out of range when referring to "
              "%ld!=block[%ld]\n",
              s, l);
        fflush(stdout);
#endif
  ...
  for (j = 0; j < nblocks; j++) {
    ...
    while (r < mrows) {
      ...
      if (l < 0 || l >= n)
        printf(
            " 8. scalar index out of range when referring to %ld=block[%ld]\n",
            s, l);
      fflush(stdout);
      if (s < 0 || s >= nblocks)
        printf(
            " 8. block index out of range when referring to %ld=block[%ld]\n",
            s, l);
      fflush(stdout);
#endif
      ...
      while (r2 < mrows) {
        ...
        if (l < 0 || l >= n)
          printf(
              " 9. scalar index out of range when referring to "
              "%ld!=block[%ld]\n",
              s, l);
        fflush(stdout);
        if (s < 0 || s >= nblocks)
          printf(
              " 9. block index out of range when referring to "
              "%ld!=block[%ld]\n",
              s, l);
        fflush(stdout);
#endif
...
#pragma omp parallel for shared(                                               \
    nblocks, iLT, idxl_src, idxposl_src, idxl2_src, iL, block, iL_new, BiD,    \
    vall_src, size_vall_src, vall2_src, size_vall2_src, iLTsize, iLTstart,     \
    idxb_src, idxbpos_src,                                                     \
    iLTbegin_src) private(mythreadnum, ia, idxl, idxposl, idxl2, ncols, mrows, \
                          ja, i, cnt, r, l, s, r2, ja2, cnt2, jj, ii, i2, tt,  \
                          ttt, ss, i3, t, k, a, pr, vall, vall2, transa,       \
                          transb, alpha, beta, idxb, idxbpos, iLTbegin, cntb,  \
                          BiD_inplace)
  for (j = 0; j < nblocks; j++) {
    // who am I
    mythreadnum = omp_get_thread_num();
    ...
    // j of iL^T*BiD*iL
    idxl2 = idxl2_src[mythreadnum];
    // idxb is used to store the block row indices of block column j of
    // iL^T*BiD*iL
...
#ifdef PRINT_INFO2
    printf(
        "block column %ld, init with indices of the diagonal block (%ld,%ld)\n",
        j, j, j);
    fflush(stdout);
#endif
#ifdef PRINT_CHECK
    for (i = 0; i < nblocks; i++) idxl2[i] = -1;
#endif
...
#endif
    ja = iL.rowind[j];
    // first pass, determine nonzero block-diagonal row pattern of
    // iL^T*BiD*iL{j}
    cntb = 0;
#ifdef PRINT_CHECK
    ...
    printf("cnt=%ld\n", cnt);
    printf("idxb\n");
    for (i = 0; i < cntb; i++) printf("%4ld", idxb[i] + 1);
    printf("\n");
    fflush(stdout);
    printf("idxbpos\n");
    for (i = 0; i < cntb; i++) printf("%4ld", idxbpos[idxb[i]] - 1);
    printf("\n");
    fflush(stdout);
    printf("idxl2\n");
    for (i = 0; i < cntb; i++) printf("%4ld", idxl2[idxb[i]]);
    printf("\n");
    fflush(stdout);
    printf("iLTbegin\n");
    for (i = 0; i < nblocks; i++) printf("%4ld", iLTbegin[i]);
    printf("\n");
    fflush(stdout);
#endif
#ifdef PRINT_INFO2
    for (i = 0; i < cntb; i++) printf("%12ld", idxb[i] + 1);
    printf("\n");
    fflush(stdout);
#endif
    // end first pass
    ...
    if (iL_new.rowind[j] == NULL) {
      ...
      iL_new.rowind[j] =
          (integer *)realloc(iL_new.rowind[j], (size_t)cnt * sizeof(integer));
    }
    if (iL_new.valE[j] == NULL) {
      iL_new.valE[j] = (double *)malloc((size_t)cnt * ncols * sizeof(double));
    } else {
      iL_new.valE[j] = (double *)realloc(iL_new.valE[j],
                                         (size_t)cnt * ncols * sizeof(double));
    }
    ...
    for (i = 0; i < cnt; i++) printf("%4ld", iL_new.rowind[j][i] + 1);
    printf("\n");
    fflush(stdout);
#endif
    ...
    for (r = 0; r < ncols; r++, a += cnt, pr += ncols)
      memcpy(a, pr, ncols * sizeof(double));
    ...
    while (r < mrows) {
      ...
      if (l < 0 || l >= n)
        printf(
            "16. scalar index out of range when referring to %ld=block[%ld]\n",
            s, l);
      fflush(stdout);
      if (s < 0 || s >= nblocks)
        printf(
            "16. block index out of range when referring to %ld=block[%ld]\n",
            s, l);
      fflush(stdout);
#endif
      ...
      while (r2 < mrows) {
        ...
        if (l < 0 || l >= n)
          printf(
              "17. scalar index out of range when referring to "
              "%ld!=block[%ld]\n",
              s, l);
        fflush(stdout);
        if (s < 0 || s >= nblocks)
          printf(
              "17. block index out of range when referring to "
              "%ld!=block[%ld]\n",
              s, l);
        fflush(stdout);
#endif
...
#ifdef PRINT_INFO2
      printf(
          "block column %ld, use block column %ld of iL^T, build block "
          "(%ld,%ld)\n",
          j, s, s, j);
      fflush(stdout);
#endif
      ...
      vall = vall_src[mythreadnum];
      vall_src[mythreadnum] =
          (double *)realloc(vall, size_vall_src[mythreadnum] * sizeof(double));
      // for technical reasons, init vall with 0
      vall = vall_src[mythreadnum];
      for (i2 = 0; i2 < jj; i2++) vall[i2] = 0.0;
...
#ifdef PRINT_INFO1
        printf(
            "BiD{%ld} need not be copied, since %ld...%ld are %ld contiguous "
            "indices\n",
            s + 1, ja[r] + 1, ja[r2 - 1] + 1, r2 - r);
        fflush(stdout);
#endif
      } else
        BiD_inplace = 0;
      ...
      if (ncols == 1 && r2 - r == 1) {
        ...
        vall2 = pr + jj * (ja[r] - ii);
        alpha = iL.valE[j][r];
        for (l = 0; l < jj; l++) *vall++ = alpha * *vall2++;
        vall -= jj;
      }
      // block case use level-3 BLAS
        ...
        if (BiD_inplace) {
          ...
          for (l = r; l < r2; l++, vall2 += jj) {
            ...
            memcpy(vall2, pr + jj * (ja[l] - ii), jj * sizeof(double));
          }  // end for l
          vall2 = vall2_src[mythreadnum];
...
#ifdef _BLAS_LAPACK_32_
        int myjj = jj, myncols = ncols, myl = l, mymrows = mrows;
        dgemm_(transa, transb, &myjj, &myncols, &myl, &alpha, vall2, &myjj,
               iL.valE[j] + r, &mymrows, &beta, vall, &myjj, 1, 1);
#else
        dgemm_(transa, transb, &jj, &ncols, &l, &alpha, vall2, &jj,
               iL.valE[j] + r, &mrows, &beta, vall, &jj, 1, 1);
#endif
      ...
      for (l = 0; l < ncols; l++, a += cnt, vall += jj) {
        ...
        memcpy(a, vall, jj * sizeof(double));
      }  // end for l
      vall = vall_src[mythreadnum];
      ...
      for (i3 = iLTbegin[s]; i3 < ia[s]; i3++) {
        ...
        if (t >= j) {
...
#ifdef PRINT_INFO1
            printf(
                "block row %ld of BiD{%ld}*iL{%ld} need not be copied, since "
                "%ld...%ld are %ld contiguous row indices\n",
                t + 1, s + 1, j + 1, ja2[0] + 1, ja2[jj - 1] + 1, jj);
            fflush(stdout);
#endif
          } else {
            BiD_inplace = 0;
          ...
          if (ncols == 1 && jj == 1) {
            ...
            vall = vall_src[mythreadnum];
#ifdef _BLAS_LAPACK_32_
            int mynblockcol = iL.nblockcol[t], mynblockrow = iL.nblockrow[t],
                myttt = ttt;
            daxpy_(&mynblockcol, vall2, a, &mynblockrow,
                   iL_new.valE[j] + idxl2[t], &myttt);
#else
            daxpy_(&(iL.nblockcol[t]), vall2, a, &(iL.nblockrow[t]),
                   iL_new.valE[j] + idxl2[t], &ttt);
#endif
          }
          // block case use level-3 BLAS
          else {
...
#ifdef _BLAS_LAPACK_32_
                int myjj = jj, mykk,
                    *myja2 = (int *)malloc((size_t)jj * sizeof(int));
                for (mykk = 0; mykk < myjj; mykk++) myja2[mykk] = ja2[mykk];
                cblas_dgthr(myjj, vall, vall2, myja2);
                free(myja2);
#else
                cblas_dgthr(jj, vall, vall2, ja2);
#endif
                vall2 += jj;
#else
...
#ifdef PRINT_INFO2
            printf(
                "block column %ld, use block column %ld of iL^T, block %ld, "
                "update block (%ld,%ld)\n",
                j, s, t, t, j);
            fflush(stdout);
#endif
            ...
            if (BiD_inplace) {
#ifdef _BLAS_LAPACK_32_
              int mynblockcol = iL.nblockcol[t], myncols = ncols, myjj = jj,
                  mynblockrow = iL.nblockrow[t], mynblockcols = iL.nblockcol[s],
                  mycnt = cnt;
              dgemm_(transa, transb, &mynblockcol, &myncols, &myjj, &alpha, a,
                     &mynblockrow, vall2, &mynblockcols, &beta,
                     iL_new.valE[j] + idxl2[t], &mycnt, 1, 1);
#else
              dgemm_(transa, transb, &(iL.nblockcol[t]), &ncols, &jj, &alpha, a,
                     &(iL.nblockrow[t]), vall2, &(iL.nblockcol[s]), &beta,
                     iL_new.valE[j] + idxl2[t], &cnt, 1, 1);
#endif
            } else {
#ifdef _BLAS_LAPACK_32_
              int mynblockcol = iL.nblockcol[t], myncols = ncols, myjj = jj,
                  mynblockrow = iL.nblockrow[t], mycnt = cnt;
              dgemm_(transa, transb, &mynblockcol, &myncols, &myjj, &alpha, a,
                     &mynblockrow, vall2, &myjj, &beta,
                     iL_new.valE[j] + idxl2[t], &mycnt, 1, 1);
#else
              dgemm_(transa, transb, &(iL.nblockcol[t]), &ncols, &jj, &alpha, a,
                     &(iL.nblockrow[t]), vall2, &jj, &beta,
                     iL_new.valE[j] + idxl2[t], &cnt, 1, 1);
#endif
            }
#ifdef PRINT_INFO2
  ...
  // step 3: rescale iL_new and sparsify it when transferring it to W

  // next compute (P^TSP) (iL^T BiD iL) (P^TSP) = (P^TSP) iL_new (P^TSP)
  // iL_new is block lower triangular

  if (SL != NULL) {
#pragma omp parallel for shared(nblocks, iL, iL_new, p, SL) private( \
    ncols, ia, mrows, ja, i, a, k, jj, r, buff, alpha)
    for (j = 0; j < nblocks; j++) {
      ncols = iL.nblockcol[j];
      // short cut
      ia = iL.colind[j];
...
#ifdef _BLAS_LAPACK_32_
        int mymrows = mrows, myi = i;
        dscal_(&mymrows, SL + r, a, &myi);
#else
        dscal_(&mrows, SL + r, a, &i);
#endif
      }  // end for k
  ...
  valr = (double *)vall_src[0];
#pragma omp parallel for shared(nblocks, iL, iL_new, valr) private( \
    ncols, ia, mrows, ja, a, k, ii, i)
  for (j = 0; j < nblocks; j++) {
    ncols = iL.nblockcol[j];
    // short cut
    ia = iL.colind[j];
  ...
  // copy   P (P^TSP) (iL^T BiD iL) (P^TSP) P^T = P iL_new P^T   to W and
  // sparsify during copying, we sparsify each off-diagonal entry using
  // |W(i,j)| > epsilon * sqrt(|W(i,i)|*|W(j,j)|)
  FreeSparse(&W);
  W.nr = W.nc = n;
  // ncol is set to 0 in order to count the nz
  W.ncol = (integer *)calloc((size_t)n, sizeof(integer));
  W.rowind = (integer **)malloc((size_t)n * sizeof(integer *));
  W.val = (double **)malloc((size_t)n * sizeof(double *));

  // sparsify and copy to W
#pragma omp parallel for shared(nblocks, iL, iL_new, mytol, valr, p, n,    \
                                W) private(ncols, ia, mrows, ja, a, k, ii, \
                                           cnt, i, jj, r, s)
  for (j = 0; j < nblocks; j++) {
    ncols = iL.nblockcol[j];
    // short cut
    ...
    for (k = 0; k < ncols; k++, a += mrows) {
      ...
      for (i = 0; i < mrows; i++) {
        ...
        jj = ja[i];
        if (jj >= ii) {
          if (FABS(a[i]) > mytol * valr[ii] * valr[jj] || ii == jj) cnt++;
        }  // end if jj>=ii
      }    // end for i
...
#endif
      W.ncol[r] = cnt;
      W.rowind[r] = (integer *)malloc((size_t)cnt * sizeof(integer));
      W.val[r] = (double *)malloc((size_t)cnt * sizeof(double));
      ...
      for (i = 0; i < mrows; i++) {
        ...
        if (jj >= ii) {
          if (FABS(a[i]) > mytol * valr[ii] * valr[jj] || ii == jj) {
            // transfer permutated index to W
            ...
            cnt++;
             ...
             // |iL_new(jj,ii)|>mytol*sqrt(|iL_new(ii,ii)|*|iL_new(jj,jj)|) or
             // ii=jj
        }    // end if jj>=ii
      }      // end for i
    }        // end for k
  ...
  fflush(stdout);
  // PrintSparse(&W);fflush(stdout);
#endif

  // release iL
  for (j = 0; j < nblocks; j++) {
    if (iL.rowind[j] != NULL) free(iL.rowind[j]);
    if (iL.valE[j] != NULL) free(iL.valE[j]);
  }  // end for j
  free(iL.nblockrow);
  free(iL.rowind);
  free(iL.valE);
  ...
  // release iL_new
  for (j = 0; j < nblocks; j++) {
    if (iL_new.rowind[j] != NULL) free(iL_new.rowind[j]);
    if (iL_new.valE[j] != NULL) free(iL_new.valE[j]);
  }  // end for j
  ...
  // 4. complete W with its transpose

  // now supplement W with W^T and store it in W
  // counter for nnz of W^T excluding diag(W)
  idxl = idxl_src[0];
  for (j = 0; j < n; j++) idxl[j] = 0;
  // short cut
  ia = W.ncol;
  // first pass: compute additional amount of memory
  for (j = 0; j < n; j++) {
    mrows = ia[j];
    // short cut
    ja = W.rowind[j];
...
#ifdef PRINT_INFO2
  printf("1. pass completed, W supplemented with its transpose\n");
  fflush(stdout);
#endif

  // intermediate step, re-allocate memory
  for (j = 0; j < n; j++) {
    k = ia[j] + idxl[j];
    W.rowind[j] = (integer *)realloc(W.rowind[j], (size_t)k * sizeof(integer));
    W.val[j] = (double *)realloc(W.val[j], (size_t)k * sizeof(double));
    // re-set counter
    idxl[j] = 0;
  }  // end for j
#ifdef PRINT_INFO2
  printf("memory allocation completed, W supplemented with its transpose\n");
  fflush(stdout);
#endif

  // second pass: fill additional entries
  for (j = 0; j < n; j++) {
    // associated number of rows in column j
    mrows = ia[j];
    // short cuts
    ja = W.rowind[j];
    a = W.val[j];
    for (k = 0; k < mrows; k++) {
      i = ja[k];
      // fill off-diagonal entry
      if (i != j) {
        // associated number of rows in column i
        ii = ia[i];
        // current additional number of entry for W^T excluding diag(W)
        jj = idxl[i];
        // insert entry at the end of column i
        W.rowind[i][ii + jj] = j;
        W.val[i][ii + jj] = a[k];
        // increase counter for the number of additional entries
        idxl[i] = jj + 1;
      }
    }  // end for k
  }    // end for j
#ifdef PRINT_INFO
  printf("W supplemented with its transpose\n");
  fflush(stdout);
#endif
  ...
  for (j = 0; j < n; j++) ia[j] += idxl[j];
  nnz = 0;
  for (j = 0; j < n; j++) nnz += ia[j];
  W.nnz = nnz;
#ifdef PRINT_INFOnnz
  printf("BNINV: final (nonsymmetric) approximate inverse computed, nnz=%ld\n",
         nnz);
  fflush(stdout);
  // Sparse(W);fflush(stdout);
#endif

  // sort columns of W
#pragma omp parallel for shared(n, idxl_src, ia, W) private(mythreadnum, idxl)
  for (j = 0; j < n; j++) {
    // who am I
    mythreadnum = omp_get_thread_num();
    ...
    qsort1_(W.val[j], W.rowind[j], idxl, ia + j);
  }  // end for j
     // end omp parallel for
#ifdef PRINT_INFO
  printf("BNINV: W sorted\n");
  fflush(stdout);
#endif
  ...
  printf("Total BNINV time %12.4le\n\n", time_bfspai);
...
void SQUIC::PrintBlockSparse(SparseBlockMatrix *B) {
  ...
  for (j = 0; j < nblocks; j++) {
    ...
    printf("            ");
    for (k = 0; k < ncols; k++) printf("%12ld", ja[k] + 1);
    printf("\n");
    fflush(stdout);
    if (valD != NULL) {
      for (l = 0; l < ncols; l++) {
