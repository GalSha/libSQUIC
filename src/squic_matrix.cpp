...
void SQUIC::run(double Lambda, SparseMatrix *LambdaMatrix, double drop_tol, int maxIter, double term_tol) {
	run_core(Lambda, LambdaMatrix, drop_tol, maxIter, term_tol);
} // end SQUIC::run

// randomized version, entry point WITH initial guesses
void SQUIC::run(double Lambda, SparseMatrix *LambdaMatrix, double drop_tol, int maxIter, double term_tol, SparseMatrix *X0, SparseMatrix *W0) {
	FreeSparse(&X);
	if (X0 == NULL)
...
void SQUIC::run_core(double Lambda, SparseMatrix *LambdaMatrix, double drop_tol, int maxIter, double term_tol) {
	...
	if (RunTimeConfig.verbose == 0) {
		MSG("#SQUIC Version %.2f : p=%d n=%d lambda=%.2e nnz(M)/p=%.2e max_iter=%d term_tol=%.2e drop_tol=%.2e",
		    SQUIC_VER,
		    integer(p),
		    integer(n),
		    double(Lambda),
	...
	for (; NewtonIter <= maxNewtonIter; NewtonIter++) {
		...
		if (NewtonIter == 1 && IsDiag(&X)) {
			...
			for (integer cdSweep = 1; cdSweep <= 1 + NewtonIter / 3; cdSweep++) {
				...
				// (RunTimeConfig.block_upd == COLLECTIVE_CDU)
					BlockCoordinateDescentUpdate(Lambda, LambdaMatrix, &D,
					                             idx, idxpos, val,
			...
			if (nn > 0) {
				...
				if (ierr) {
					...
					if (RunTimeConfig.verbose > 1) {
							...
							MSG("- Cholesky successful: time=%e nnz(L)/p=%e logdetX=%e\n", Stat.time_chol[nn - 1], double(Cholmod.common_parameters.lnz) / p, logdetX);
							fflush(stdout);
					}
				}
			}
		...
		if (ierr) {
			printf("!!!SQUIC(Augment X with pattern of Delta): X is nonsymmetric!!!\n");
			fflush(stdout);
		}
#endif
		// printf("augmented X:\n"); PrintSparse(X);
...
double SQUIC::DiagNewton(const double Lambda, SparseMatrix *LambdaMatrix, SparseMatrix *D) {
