...
void SQUIC::GenerateSXL3B_OMP(double *mu, double Lambda, double *nu, integer *ind, integer **idx_src, integer *idxpos) {
	/*
	...
	*/
	...
	while (cnt < p) {
		...
		#pragma omp parallel for shared(max_block, loop, z, p, S) private(cntl, zl, s, i, dbuff, rowind, val)
