...
#ifndef generates_cpp
#define generates_cpp
...
void SQUIC::GenerateSXL3B_OMP(double *mu, double Lambda, double *nu,
                              integer *ind, integer **idx_src,
                              integer *idxpos) {
  /*
    Y          n samples Y=[y_1,...y_n] from which the low-rank empirical
               covariance matrix S=1/NM1(n) sum_i (y_i-mu)(y_i-mu)^T can be
    build, where mu=1/n sum_i y_i is the mean value Y is stored by columns p
    size parameter space n          number of samples mu         mean value
    Lambda     Lagrangian parameter
    S          sparse representation of the sample covariance matrix
    nu         1/NM1(n)||Y_{i,:}-\mu||^2 diagonal entries of S sorted in
    decreasing order ind        indices 0,...,p-1 associated with. nu

    idx     \  pre-allocated auxiliary buffers of size p
    idxpos  /  initially we assume that idxpos is zero
  */

  integer i, j, k, l, s, *rowind, *idx,
      max_block,  // true maximum block size <=MAX_BLOCK_SIZE
      cnt_block, cnt_loop, *ibuff, cnt, cntl, loop;

  double *pZ,  // pointer to Z_{i,:}
      *val, dbuff, a, b, *v, *z,
      *zl,  // vectors for matrix-vector multiplication
  ...
  while (cnt < p) {
    ...
    // z has been initialized after allocation, thus there will be no 0*NaN or
    // 0*inf
    a = 1.0;
    b = 0.0;
    k = p - loop;
    ...
    dgemm_((char *)"N", (char *)"N", &myk, &mymax_block, &myn, &a, Z + loop,
           &myp, v, &myn, &b, z + loop, &myp, 1, 1);
#else
    dgemm_((char *)"N", (char *)"N", &k, &max_block, &n, &a, Z + loop, &p, v,
           &n, &b, z + loop, &p, 1, 1);
#endif
#if defined PRINT_MSGL3B || defined PRINT_MSG
...
#pragma omp parallel for shared(max_block, loop, z, p, S) private( \
    cntl, zl, s, i, dbuff, rowind, val)
